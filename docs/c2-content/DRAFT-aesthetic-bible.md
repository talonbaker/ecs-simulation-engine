...
- Crowding as a distinct proximity condition: when N NPCs are simultaneously within awareness range of each other in a confined space, high-neuroticism/low-extraversion NPCs should receive a qualitatively different stress response beyond generic irritation — something closer to *exposure* or *overwhelm*. The current model fires per-pair proximity events; crowding is an aggregate condition. Threshold N and decay rate would be tunable in SimConfig.
- Refuge spaces: certain rooms function as emotional decompression zones for specific archetypes — the IT closet for Greg, the Women's Bathroom for Donna, the Smoking Bench for anyone who needs the mask to come down. In these spaces, affected archetypes recover drives faster and pay a lower baseline exposure cost. Should refuge be annotatable per-room and per-archetype rather than derived purely from lighting and crowd density?
- The noticeability question above connects directly to masking cost: a brightly lit NPC in a high-traffic zone who is running an elevated internal drive state (grief, shame, attraction) is paying a social composure cost even without a conversation event firing. Coupling noticeability to willpower drain could produce this without a new system — the mask is harder to hold when you're visible.
