...
## PlaytestScene — Iter 3 findings

> Surfaced by PT-001-iter-3 (after the BUG-010/011 fixes in commit `a547661`). Inspector + camera glide now work; selection chain healed; save path is reported. These are the remaining gaps Talon noted.

### BUG-014: Save / load round-trip broken — Load requires engine restart which wipes the saved state

**Symptom:** `save smoketest` writes the file successfully (verified path on disk). `load smoketest` returns a path-confirmation message but **does not actually restore engine state** in-process. The doc-string and existing `Debug.Log` in `SaveLoadPanel.Load` flag this as a known v0.1 limitation — full state restore "requires engine restart." But restarting the engine drops the in-memory save buffer before the load can be consumed, producing the practical regression: the player can save, but cannot ever load.

**Severity:** **High** — save/load is an SRD §8.2 axiom. Currently the load half is broken.

**Repro:**
1. Open PlaytestScene → Play.
2. Open dev console (`~`). `scenario kill Donna` to put the world in a distinguishable state.
3. `save smoketest` — file written. Path returned.
4. `load smoketest` — response says loaded; world state does NOT change in-process.
5. Stop Play; press Play again. Donna is alive (initial state). Save effectively wiped.

**Root cause (per SaveLoadPanel.cs:123):** `Load()` is a stub. The full restore mechanism would re-instantiate the engine via `SimulationBootstrapper` with the saved JSON as the world-definition input. That requires either:
- A "restart" path that doesn't tear down the AssetDatabase (re-spin EngineHost.Awake → Start with new bootstrap inputs without exiting Play mode), OR
- A Domain Reload + Play cycle that reads the saved file as the boot input.

Neither is implemented. The engine emits/persists state correctly (per WP-3.2.0 SaveLoad round-trip-hardening); only the host-side load path is missing.

**Files relevant:**
- `ECSUnity/Assets/Scripts/UI/SaveLoadPanel.cs:111-127` — Load() stub
- `ECSUnity/Assets/Scripts/Engine/EngineHost.cs` — would need a restart-with-saved-state hook
- `APIFramework/SimulationBootstrapper.cs` — already accepts world-definition JSON; load could feed it

**Suggested fix wave:** Future feature packet `WP-PT.NN-save-load-roundtrip` or fold into the existing 3.2.0 round-trip work as a follow-up. Out of PT-001 scope.

**Workaround:** None. Save state is preserved on disk but cannot be consumed back into a running session. Useful only for offline inspection.

**Discovered in:** PT-001-iter-3.

---

### BUG-015: Room inspector doesn't open on empty-floor click

**Symptom:** Clicking on an empty floor tile (or anywhere outside an NPC / object collider) clears the current selection but doesn't open the RoomInspectorPanel. The recipe expectation is "click an empty floor tile → RoomInspectorPanel renders for the clicked room."

**Severity:** **Medium** — room inspection is a documented player verb (UX bible §3.1). NPC + object inspectors work after the BUG-010 fix; rooms specifically don't.

**Repro:**
1. Open PlaytestScene → Play.
2. Click an NPC → InspectorPanel slides in (works post-BUG-010).
3. Click an empty floor tile inside a room (e.g., the kitchen) — InspectorPanel closes; RoomInspectorPanel does NOT appear.

**Root cause (suspected):** SelectionController.Update raycasts and calls `hit.collider.GetComponentInParent<SelectableTag>()`. Empty floor tiles likely don't have any collider or `SelectableTag` attached. Either:
- Room rectangles (rendered by `RoomRectangleRenderer`) need a `BoxCollider` + `SelectableTag` with `Kind = SelectableKind.Room` to be raycast-detectable, OR
- SelectionController needs a fallback "no hit → emit a Room selection for the room containing the world-position raycast endpoint" path.

The first option is more aligned with the SelectableTag protocol the rest of the system uses. Mirrors the BUG-010 fix's pattern.

**Files relevant:**
- `ECSUnity/Assets/Scripts/Render/RoomRectangleRenderer.cs` — likely needs to add Collider + SelectableTag(Kind=Room) to room GameObjects when it creates them
- `ECSUnity/Assets/Scripts/UI/SelectionController.cs:58-90` — Update / raycast / SetSelection chain
- `ECSUnity/Assets/Scripts/BuildMode/SelectableTag.cs` — `SelectableKind.Room` enum value already exists

**Suggested fix wave:** Inline follow-up packet (similar shape to the BUG-010 fix). Likely a small change to `RoomRectangleRenderer.CreateRoomView` (or whatever its create-method is named).

**Workaround:** Use `inspect-room <room-name>` from the dev console.

**Discovered in:** PT-001-iter-3.

---

### BUG-016: InspectorPanel drill / deep buttons don't expand content

**Symptom:** When InspectorPanel renders the glance tier on an NPC, the "drill" and "deep" buttons are visible (or inferred — Talon reports "the drill and deep buttons don't display any additional information"). Clicking them does not produce a tier change; the panel stays on glance with no additional content.

**Severity:** **Medium** — three-tier disclosure is a UX bible §3.1 commitment. Glance works; drill/deep don't.

**Repro:**
1. Open PlaytestScene → Play.
2. Click an NPC → InspectorPanel renders glance tier.
3. Click "drill" button (or whatever cue is shown in the IMGUI fallback). Nothing happens.
4. Same for "deep".

**Root cause (suspected):** The IMGUI fallback in `InspectorPanel.OnGUI()` likely doesn't have a button-driven tier switcher; that logic was authored against UI Toolkit (`_drillBtn?.RegisterCallback<ClickEvent>(_ => SetTier(InspectorTier.Drill))` etc., per `InspectorPanel.cs:61-63`). The IMGUI fallback at `InspectorPanel.cs:236+` may render glance content only and skip the tier-button logic entirely.

Verification: read `InspectorPanel.OnGUI()` and check whether it includes `GUILayout.Button("Drill")` calls that invoke `SetTier()`. If absent, IMGUI fallback needs the tier-buttons added.

**Files relevant:**
- `ECSUnity/Assets/Scripts/UI/InspectorPanel.cs:236+` (the OnGUI fallback)
- `SetTier(InspectorTier)` method — already implemented for the UI Toolkit path

**Suggested fix wave:** Inline follow-up packet. Small edit to InspectorPanel.OnGUI to render tier buttons + wire them to SetTier. Bundle with BUG-015 if both are touched in the same UI-panel-polish pass.

**Workaround:** Use `inspect <npc-name>` from the dev console for full component dump (more verbose than deep tier, but covers everything).
